...
constexpr inline Response fromDialogResult(ScreenCastChooser::DialogResult r)
{
    return r == ScreenCastChooser::DialogResult::Accepted ? Success : Cancelled;
}
                       ...
                       Callback &&callback)
{
    ...
    QObject::connect(dialog, &std::remove_pointer_t<decltype(dialog)>::finished,
                     contextObject, [message, callback](const auto &result) {
